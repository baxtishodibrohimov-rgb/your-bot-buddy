...
| 5 | Image gallery + Clinical Analysis Wizard | **Schema only.** Gallery view exists on the case page (grouped by extraoral/intraoral/radiology, required-missing badges). The guided wizard UI (checklist-per-image, "next required photo") is not built. |
