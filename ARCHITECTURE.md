...
| 5 | Image gallery + Clinical Analysis Wizard | **Built.** Manual case entry (`tp_create_manual_case`), "bulut" pool upload to Supabase Storage with per-slot +/☁️ controls (`tp_assign_pool_image`), and a one-photo-at-a-time wizard (`/admin/tp/cases/$id/analysis`) following the clinic's confirmed 13-step capture order, with an interactive FDI dental chart (split per jaw) under the two occlusal steps. All 22 `tp_analysis_templates` questions are the clinic's own confirmed wording. |
| 6 | Image annotation (Konva/Fabric) | **Not started.** `tp_image_annotations` table exists (versioned JSON shapes); no canvas UI yet. |
| 7 | Master Problem List | **Done.** `tp_findings` auto-populates from `tp_analysis_answers` via a trigger + a clinic-confirmed rule table (`tp_finding_description()`), tied back to its source answer so edits/deletes keep it in sync. Manual findings (no `source_answer_id`) and confirm/reject workflow are still future work. |
